          ...
          >
            ...
            <div class="flex items-start justify-between mb-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h3 class="text-xl font-bold text-gray-800">{{ student.nickname || student.firstName }}</h3>
                  <p class="text-sm text-gray-500">{{ student.firstName }} {{ student.lastName || '' }}</p>
                  <span class="inline-block mt-1 px-2 py-1 bg-indigo-100 text-indigo-700 text-xs rounded-full font-medium">
                    {{ student.gradeName }}
                  </span>
                </div>
              </div>
...
import StudentClassSelector from '@/components/StudentClassSelector.vue';
...
console.log('Monthly enrollments:', detail.monthlyEnrollments);
console.log('Hourly group enrollments:', detail.hourlyGroupEnrollments);
console.log('Hourly individual classes:', detail.hourlyIndividualClasses);
          
          return {
            ...student,
            totalMonthlyClasses: detail.monthlyEnrollments?.length || 0,
            totalHourlyGroupClasses: detail.hourlyGroupEnrollments?.length || 0,
            totalHourlyIndividualClasses: detail.hourlyIndividualClasses?.length || 0,
...
const getUniqueSubjects = (detail) => {
  ...
  // นับจาก monthly
  detail.monthlyEnrollments?.forEach(enrollment => {
  ...
  // นับจาก hourly group
  detail.hourlyGroupEnrollments?.forEach(enrollment => {
    const key = enrollment.subjectId;
    if (subjectMap.has(key)) {
      subjectMap.get(key).classCount++;
    } else {
  ...
  // นับจาก hourly individual
  detail.hourlyIndividualClasses?.forEach(cls => {
