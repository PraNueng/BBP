<template>
  ...
  >
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col">
      <!-- Header -->
      <div class="sticky top-0 bg-gradient-to-r from-indigo-600 to-purple-600 p-6 rounded-t-2xl z-10">
        <div class="flex-col">
          <div class="flex items-center justify-center">
            <div class="flex items-center gap-3">
              ...
              <h2 class="text-3xl font-bold text-white">ประวัติการแก้ไข</h2>
            </div>
          </div>
          <div>
            <button 
        ...
        <p v-if="studentName" class="text-indigo-100 mt-2 text-lg flex justify-center">
          ชื่อ: <span class="font-semibold pr-4">{{ nickname }} ({{ studentName }})</span>
          ชั้น: <span class="font-semibold">{{ grade }}</span>
        </p>
      </div>
      ...
      <div v-else class="flex-1 overflow-y-auto p-6">
        <ul v-if="filteredHistory.length > 0" class="space-y-4">
          <li v-for="(h, i) in filteredHistory" :key="i" 
            class="bg-gradient-to-r from-gray-50 to-blue-50 p-5 rounded-xl border-l-4 border-indigo-500 shadow-sm hover:shadow-md transition-all"
          >
              ...
              <div class="flex-1">
                ...
                <div class="flex items-center gap-4 text-sm text-gray-600">
                  <div class="flex items-center gap-1">
                    ...
                    <span class="font-medium">{{ h.updatedBy || 'ไม่ทราบ' }}</span>
                  </div>
                  <div class="flex items-center gap-1">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div v-else class="text-center py-16">
          ...
          <p class="text-gray-400 text-sm mt-2">ประวัติจะแสดงเมื่อมีการแก้ไขข้อมูลนักเรียน</p>
        </div>
      </div>
...
const getFieldLabel = (fieldName) => {
  const labels = {
    ...
    schoolName: 'โรงเรียน',
    grade: 'ชั้นเรียน',
    phoneStudent: 'เบอร์นักเรียน',
    phoneParent: 'เบอร์ผู้ปกครอง',
...
const filteredHistory = computed(() => {
  return props.history.filter(h => allowedFields.includes(h.fieldName));
});
...
const formatValue = (fieldName, value) => {
  ...
  if (fieldName === 'isActive') {
    return value === 'true' || value === true ? 'ใช้งาน' : 'ไม่ใช้งาน';
  }
  
    if (fieldName === 'classAdded' && typeof value === 'string') {
