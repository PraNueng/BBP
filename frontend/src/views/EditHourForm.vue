    ...
    <div class="container mx-auto px-4 lg:px-8 py-8 max-w-4xl">
      ...
      <div v-else class="bg-white rounded-b-2xl shadow-2xl p-8">
        <form @submit.prevent="handleUpdate" class="space-y-6">
          ...
          <!-- แสดงชั้น + วิชา (Read-only) -->
          <!-- <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-gray-700 font-semibold mb-2">ระดับชั้น</label>
              <div class="w-full px-4 py-3 bg-gray-100 border border-gray-300 rounded-lg text-gray-700">
                {{ formData.grade || '-' }}
              </div>
            </div>
...
onMounted(async () => {
  try {
    ...
    const response = await api.get(`/hour-forms/${id}`);
    const data = response.data;

    formData.value = {
      classId: data.classId,
      classType: data.classType,
      className: data.className || data.classDisplayName || '-',
      grade: data.grade || '-',
    ...
    // เก็บ original data สำหรับเช็คการเปลี่ยนแปลง
...
const checkChanged = () => {
  ...
  // เช็คเฉพาะ fields ที่แก้ได้
  const editableFields = ['content', 'hoursTaught', 'studentsPresent', 'studentsAbsent', 'remark'];
...
const handleUpdate = async () => {
  ...
  const requiredFields = ['content', 'teachingDate', 'hoursTaught', 'studentsPresent', 'studentsAbsent'];
  let hasError = false;
  ...
  try {
    ...
    await api.put(`/hour-forms/${id}`, dataToSubmit);
