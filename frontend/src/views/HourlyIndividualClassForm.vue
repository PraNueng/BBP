<template>
  <div class="mb-6 p-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-300">
    <h3 class="text-xl font-bold text-blue-800 mb-4 flex items-center gap-2">
      <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
      </svg>
      เพิ่มคลาสตัวต่อตัวใหม่
    </h3>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <!-- Student -->
        <div>
          <label class="block text-sm font-semibold text-gray-700 mb-2">นักเรียน *</label>
          <select
      ...
      <button
        ...
        class="w-full px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white font-bold rounded-lg hover:from-blue-600 hover:to-blue-700 transition-all shadow-lg disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed flex items-center justify-center gap-2"
      >
...
const handleSubmit = async () => {
  ...
  try {
    ...
    emit('success', 'เพิ่มคลาสตัวต่อตัวสำเร็จ!');
    ...
    console.error('Error creating hourly individual class:', error);
