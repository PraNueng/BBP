<template>
  <div>
    ...
    <div v-else-if="classes.length === 0" class="text-center py-8 text-gray-500">
      ...
      <p>{{ hasFilters ? 'ไม่พบคลาสที่ตรงกับเงื่อนไข' : 'ยังไม่มีคลาสรายเดือน' }}</p>
    </div>
    ...
    <div v-else class="space-y-3">
      <div
        ...
        :key="cls.id"
        class="bg-white border-2 border-gray-200 rounded-xl p-4 hover:border-green-300 transition-all"
      >
...
const classes = ref([]);
const allClasses = ref([]); // เก็บคลาสทั้งหมด
...
const loadClasses = async () => {
  ...
  try {
    ...
    const response = await api.get('/monthly-classes', { params });
...
const handleToggleActive = async (cls) => {
  try {
    await api.patch(`/monthly-classes/${cls.id}/toggle-active`);
