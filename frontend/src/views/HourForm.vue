<template>
  <div class="min-h-screen bg-gradient-to-br from-yellow-50 to-yellow-100">
    ...
    <div class="container mx-auto px-4 lg:px-8 py-8 max-w-4xl">
      <div class="bg-gradient-to-r from-yellow-500 to-yellow-700 rounded-t-2xl shadow-xl text-center py-10">
        <h1 class="text-xl font-semibold text-white mb-2">แบบฟอร์มบันทึกการสอน</h1>
        <h1 class="text-5xl font-extrabold text-white drop-shadow-md">รายชั่วโมง</h1>
      </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div>
            ...
            >
              ...
              <optgroup label="กลุ่มรวม">
                <option
                  v-for="cls in groupClasses"
                  :key="'group-' + cls.id"
                  :value="JSON.stringify({ id: cls.id, type: 'hourly_group', subjectId: cls.subjectId, grade: cls.grade })"
                >
                  {{ cls.className }}
                </option>
              </optgroup>
              <optgroup label="PV-เดี่ยว / PV-กลุ่ม">
                <option
                  v-for="cls in individualClasses"
                  :key="'individual-' + cls.id"
                  :value="JSON.stringify({ id: cls.id, type: 'hourly_individual', subjectId: cls.subjectId, grade: cls.grade })"
                >
                  {{ cls.className }}
                </option>
              </optgroup>
...
const loadClasses = async () => {
  try {
    ...
    const groupResponse = await api.get(`/hourly-group-classes?tutorId=${tutorId}&active=true`);
    groupClasses.value = groupResponse.data.map(cls => ({
      id: cls.id,
    ...
    const individualResponse = await api.get(`/hourly-individual-classes?tutorId=${tutorId}&active=true`);
    individualClasses.value = individualResponse.data.map(cls => ({
      id: cls.id,
      className: cls.className,
      subjectId: cls.subject?.id || cls.subjectId,
      subjectName: cls.subject?.subjectName || cls.subjectName || '-',
...
const handleClassChange = () => {
  ...
  try {
    ...
    formData.value.classType = classData.type;
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await api.post('/hour-forms', dataToSubmit);

    if (response.status === 201) {
      modalType.value = 'success';
      modalMessage.value = 'บันทึกข้อมูลสำเร็จ! ชั่วโมงถูกบันทึกให้นักเรียนทุกคนในคลาสแล้ว';
      showModal.value = true;
    }
  } catch (error) {
